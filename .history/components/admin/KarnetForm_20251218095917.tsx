...
import { createKarnet, updateKarnet } from '@/app/(site)/panel/actions'
...
export function KarnetForm({ initialData, isNew = false }: KarnetFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isNew) {
        await createKarnet(formData)
      } else {
        await updateKarnet(initialData.id, formData)
      }
      router.push('/panel/karnety')
      router.refresh()
    } catch (err: any) {
