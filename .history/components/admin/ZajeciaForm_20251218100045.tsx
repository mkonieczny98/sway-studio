...
import { createZajecia, updateZajecia } from '@/app/(site)/panel/actions'
...
export function ZajeciaForm({ initialData, isNew = false }: ZajeciaFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isNew) {
        await createZajecia(formData)
      } else {
        await updateZajecia(initialData.id, formData)
      }
      router.push('/panel/zajecia')
      router.refresh()
    } catch (err: any) {
        ...
        <div className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {formData.image && (
                <div className="rounded-lg overflow-hidden border relative h-40">
