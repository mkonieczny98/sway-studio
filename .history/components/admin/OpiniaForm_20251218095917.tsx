...
import { createOpinia, updateOpinia } from '@/app/(site)/panel/actions'
...
export function OpiniaForm({ initialData, isNew = false }: OpiniaFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isNew) {
        await createOpinia(formData)
      } else {
        await updateOpinia(initialData.id, formData)
      }
      router.push('/panel/opinie')
      router.refresh()
    } catch (err: any) {
            ...
            <CardContent>
              <div className="rounded-lg bg-muted/50 p-4">
                ...
                <p className="text-sm text-muted-foreground italic">
                  &ldquo;{formData.content || 'Treść opinii...'}&rdquo;
                </p>
              </div>
            </CardContent>
          </Card>
